...
class PluginRouteResolverTest {
  ...
  void setUp() {
    ...
    // error response can exceed the production 3-second connection/read timeout on the
    ...
    // fixture exactly 2x connection/read time; all response assertions remain
    // production-equivalent and no request is retried by the test.
    ...
    ProxyExecutionService proxyExecutionService =
        new ProxyExecutionService(
            new RestClientProxyExecutionAdapter(
                RestClient.builder(), Duration.ofSeconds(6), Duration.ofSeconds(6)));
